function dtec=My_Detrending(vtec,time_win,min_amp,max_amp,extend_or_not)
...
extension_L=extend_or_not*length(vtec); 
vtec=My_Sig_Extension(vtec,{'asymw'},extension_L); %signal extension
% dtec=vtec-movmean(vtec,round(seconds(time_win)/30*0.8));
